...
kb_products.add(InlineKeyboardButton(text='Продукт 1', callback_data='product_buying'),
                InlineKeyboardButton(text='Продукт 2', callback_data='product_buying'),
                InlineKeyboardButton(text='Продукт 3', callback_data='product_buying'),
                InlineKeyboardButton(text='Продукт 4', callback_data='product_buying'))

# получение списка продуктов из базы данных
products: list[tuple] = get_all_products()
...
async def get_buying_list(call: CallbackQuery):  # запрос обратного вызова от кнопки в сообщении
    for product_id, title, description, price in products:
        with open('files/1.jpg', 'rb') as img:
            await call.message.answer_photo(img, f'Название: {title} | Описание: {description} | Цена: {price}')
...
# РЕГИСТРАЦИЯ ------------------------------------------------------------------------------------------------------- *
# выполнение функции только при получении сообщения Регистрация
...
async def sign_up(call: CallbackQuery) -> None:
    await call.message.answer('Введите имя пользователя (только латинский алфавит).')  # выводит сообщение в телеграмме
    await call.answer()  # завершение срабатывания кнопки (разблокирует повторное нажатие кнопки)
    await RegistrationState.username.set()

@dp.message_handler(state=RegistrationState.username)
async def set_username(message: types.Message,  # сообщение, написанное пользователем
                       state: FSMContext) -> None:  # FSMContext - машина состояний (Finite State Machine)
    if not is_included(message.text):
        await state.update_data(username=message.text)  # запись значения в словарь под ключом 'username'
...
async def set_email(message: types.Message,  # сообщение, написанное пользователем
                    state: FSMContext) -> None:  # FSMContext - машина состояний (Finite State Machine)
...
async def set_age(message: types.Message,  # сообщение, написанное пользователем
                  state: FSMContext) -> None:  # FSMContext - машина состояний (Finite State Machine)
